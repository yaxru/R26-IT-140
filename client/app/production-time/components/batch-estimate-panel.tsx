...
export default function BatchEstimatePanel() {
  ...
  useEffect(() => {
    const fetchLines = async () => {
      const { data } = await supabase.from("vw_line_assignments").select("line_id");
      if (data) {
        const uniqueLines = Array.from(new Set(data.map((r) => r.line_id))).sort();
  ...
  // 2. Auto-fetch human workers when the line changes
  useEffect(() => {
    const fetchWorkerCount = async () => {
      ...
      if (count !== null) {
        setForm(prev => ({ ...prev, no_of_workers: count.toString() }));
      }
    };
    fetchWorkerCount();
    ...
    <main className="min-h-screen bg-[#F8F8F8] dark:bg-[#030C08] flex flex-col text-[#242424] dark:text-zinc-200">
      <section className="bg-white dark:bg-[#111113] border-b border-[#EAEAEA] dark:border-zinc-800 px-6 lg:px-8 py-6 flex items-start justify-between gap-4 shrink-0">
        <div>
          ...
          <h1 className="text-xl font-bold tracking-tight text-[#242424] dark:text-zinc-100">Set Line Productivity Targets</h1>
          <p className="text-xs text-[#5F5F5F] dark:text-zinc-400 mt-1.5 leading-relaxed">
            Run the AI prediction model to calculate and set the dynamic piece targets for the factory floor.
          </p>
        </div>
      </section>
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        >
          <div className="px-6 py-4 border-b border-[#EAEAEA] dark:border-zinc-800 bg-[#F8F8F8] dark:bg-zinc-900/40 flex items-center justify-between">
            <div className="text-[10px] font-bold uppercase tracking-widest text-[#5F5F5F] dark:text-zinc-400">01 / Floor Inputs</div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <input type="number" min="1" step="0.5" value={form.no_of_workers} onChange={(e) => updateField("no_of_workers", e.target.value)} className={`${inputClass} bg-blue-50/50 dark:bg-blue-900/10`} />
                  <span className="absolute right-3 top-8 text-[9px] font-mono text-blue-600 dark:text-blue-400">Auto-fetched</span>
                </div>
                <div>
          ...
          <div className="px-6 lg:px-8 py-5 border-t border-[#EAEAEA] dark:border-zinc-800 flex items-center justify-between gap-4 bg-[#F8F8F8] dark:bg-zinc-900/20 shrink-0">
            <p className="text-[10px] font-mono text-[#9A9A9A] dark:text-zinc-500">Submitting will update live floor targets.</p>
        ...
        <section className="lg:w-1/2 bg-white dark:bg-[#111113] flex flex-col" aria-live="polite">
          <div className="px-6 py-4 border-b border-[#EAEAEA] dark:border-zinc-800 bg-[#F8F8F8] dark:bg-zinc-900/40 flex items-center justify-between">
            <div className="text-[10px] font-bold uppercase tracking-widest text-[#5F5F5F] dark:text-zinc-400">02 / Output · Floor Assignment</div>
            {result && (
              <div className="flex items-center gap-3">
                <span className="text-[9px] font-mono text-[#9A9A9A] uppercase tracking-widest">Updated {lastRunAt}</span>
                <span className={`text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 ${result.targets_updated ? "bg-[#E6F1EC] text-[#1A7C4B] dark:bg-[#0A321E] dark:text-[#47966F]" : "bg-[#F1F1F1] text-[#9A9A9A] dark:bg-zinc-800"}`}>
                  {result.targets_updated ? "Floor Targets Locked" : "Not Saved"}
                </span>
              </div>
            )}
          </div>
          ...
          {result ? (
            <div className="p-6 lg:p-8 flex flex-col gap-6 flex-1">
              ...
              <div className="border-l-2 border-[#1A7C4B] pl-4 py-1">
                <p className="text-[10px] font-bold uppercase tracking-widest text-[#1A7C4B] dark:text-[#47966F] mb-1">✦ Floor Update Success</p>
                <p className="text-xs text-[#5F5F5F] dark:text-zinc-400 leading-relaxed">
                  The AI prediction has been locked in. All operators on {selectedLine} will now see their dynamic piece targets on their tablets.
                </p>
              </div>
            </div>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
              ...
              <div>
                <h3 className="font-bold text-sm text-[#242424] dark:text-zinc-100 mb-1">No target set</h3>
                <p className="text-xs text-[#9A9A9A] dark:text-zinc-500">Run the model to set the physical piece targets for the selected line.</p>
              </div>
            </div>
          )}
        </section>
