...
export default function WorkforcePage() {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          try {
            ...
            const workers: ParsedWorker[] = rawData.map((row) => ({
  ...
  const handleImportWorkers = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < previewData.length; i += CHUNK_SIZE) {
        ...
        if (invokeError) throw new Error(`Batch ${chunkIndex} failed: ${invokeError.message}`);
      ...
      setImportResult({
        success: allSuccess,
        failed: allFailed,
        summary: { total: previewData.length, created: totalCreated, failed: totalFailed }
      });
  ...
  const groupedWorkers = previewData.reduce((acc, worker) => {
  ...
  const CredentialCard = ({ worker }: { worker: WorkerAccountCreated }) => (
    <div className="bg-white dark:bg-zinc-900 border-2 border-zinc-800 p-4 text-center w-64 break-inside-avoid mb-6">
      <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-2">Worker Credentials</p>
      ...
      <div className="bg-emerald-100 dark:bg-emerald-900 p-2">
        <p className="text-[10px] text-emerald-700 dark:text-emerald-300 font-mono uppercase">PIN (Keep Secret)</p>
  ...
  return (
    <div className="flex flex-col h-full p-6 gap-6">
        ...
        <div className="space-y-4">
          <div
            ...
            className={`border-2 border-dashed rounded p-8 text-center transition-colors ${
              isDragging ? "border-emerald-500 bg-emerald-500/5" : "border-zinc-300 dark:border-zinc-700 hover:border-emerald-500/50"
            }`}
          >
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-zinc-900 dark:text-zinc-100">Drag and drop your CSV file here</p>
            ...
            <button onClick={() => fileInputRef.current?.click()} className="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded transition-colors">
              Select CSV File
            </button>
          </div>
          ...
          {previewData.length > 0 && !showImportModal && (
            <div className="flex items-center justify-between bg-blue-500/10 border border-blue-500/30 p-3 rounded text-sm">
              <div>
                <p className="text-blue-600 dark:text-blue-400 font-medium">✓ {previewData.length} worker(s) ready to import</p>
                <p className="text-xs text-blue-500 mt-1">Click below to review the parsed data</p>
              </div>
              <button onClick={() => setShowImportModal(true)} className="px-3 py-1.5 bg-blue-600 text-white text-xs rounded hover:bg-blue-700">
                Review Data
              </button>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-red-500/10 border border-red-500/50 p-3">
                  ...
                  <p className="text-2xl font-bold text-red-400">{importResult.summary.failed}</p>
                </div>
                <div className="bg-blue-500/10 border border-blue-500/50 p-3">
                  ...
                  <p className="text-2xl font-bold text-blue-400">{importResult.summary.total}</p>
                </div>
              </div>

              {importResult.failed.length > 0 && (
                <div className="bg-red-500/5 border border-red-500/30 p-4">
                  <p className="text-xs font-mono text-red-400 uppercase mb-3">Failed Records</p>
              ...
              {importResult.success.length > 0 && (
                <div className="mt-8">
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-xs font-mono text-zinc-500 uppercase">Credential Cards (Print & Distribute)</p>
                    <button onClick={() => window.print()} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium transition-colors">
                      Print Credentials
                    </button>
                  </div>
                  <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-6 print:columns-4 space-y-6">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-[#111113] border border-zinc-200 dark:border-zinc-800/60 shadow-xl max-w-4xl w-full max-h-[85vh] flex flex-col">
            ...
            <div className="bg-white dark:bg-[#111113] border-b border-zinc-200 dark:border-zinc-800/60 p-5 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Review & Confirm Upload</h3>
                <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-1">{previewData.length} total workers mapped.</p>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={clearList} disabled={importing} className="text-xs text-red-500 hover:text-red-600 disabled:opacity-50 font-medium px-3 py-1.5 border border-red-500/30 rounded">
                  Clear List
                </button>
                <button onClick={() => setShowImportModal(false)} className="text-zinc-400 hover:text-zinc-600 transition-colors">✕</button>
              </div>
            </div>

            <div className="p-5 overflow-y-auto flex-1 bg-zinc-50 dark:bg-zinc-900/30">
              {Object.keys(groupedWorkers).length === 0 ? (
                <div className="text-center py-10 text-zinc-500 text-sm">No data remaining.</div>
              ) : (
                Object.entries(groupedWorkers).map(([line, workers]) => (
                  <div key={line} className="mb-6 border border-zinc-200 dark:border-zinc-800/60 bg-white dark:bg-[#111113] overflow-hidden">
                    ...
                    <div className="bg-zinc-100 dark:bg-zinc-800/50 px-4 py-2 border-b border-zinc-200 dark:border-zinc-800/60">
                      <span className="text-xs font-mono font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">
                        Production Line: <span className="text-blue-600 dark:text-blue-400">{line}</span>
                      </span>
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-zinc-100 dark:border-zinc-800/40 text-zinc-500">
                            ...
                            <th className="text-left p-3 font-medium min-w-[150px]">Email</th>
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/40">
                          {workers.map((w) => (
                            <tr key={w.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/20 transition-colors group">
                              <td className="p-3 text-zinc-900 dark:text-zinc-100">{w.firstName}</td>
                              <td className="p-3 font-mono text-emerald-600 dark:text-emerald-400">{w.workerId}</td>
                              <td className="p-3 font-mono text-zinc-600 dark:text-zinc-400">{w.phoneNumber || '-'}</td>
                              <td className="p-3 text-zinc-600 dark:text-zinc-400">{w.contactEmail || '-'}</td>
            ...
            <div className="bg-white dark:bg-[#111113] border-t border-zinc-200 dark:border-zinc-800/60 p-5 flex items-center justify-between shrink-0">
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowImportModal(false)} disabled={importing} className="px-4 py-2 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 text-zinc-900 dark:text-zinc-100 text-sm font-medium rounded transition-colors disabled:opacity-50">
                  Cancel
                </button>
                <button onClick={handleImportWorkers} disabled={importing || previewData.length === 0} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded transition-colors disabled:opacity-50 flex items-center gap-2">
                  {importing ? (
      ...
      <div className="bg-blue-500/5 border border-blue-500/30 p-6 rounded mt-auto">
        <p className="text-xs font-mono text-blue-600 dark:text-blue-400 uppercase font-bold mb-2">📋 CSV Format Required</p>
        ...
        <p className="text-xs text-zinc-600 dark:text-zinc-400 font-semibold mb-1">Example:</p>
        ...
        <p className="text-xs text-blue-600 dark:text-blue-400 mt-3">
          ℹ️ Upload limits bypassed via frontend auto-batching. Worker IDs must be 4 digits.
        </p>
      </div>
