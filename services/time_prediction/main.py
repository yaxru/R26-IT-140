...
app = FastAPI(title="Opsis Time Prediction Service", version="2.0.0")
...
def _list_from_supabase(limit: int) -> list[dict]:
    try:
        response = supabase.table("prediction_runs").select("*").order("created_at", desc=True).limit(limit).execute()
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "opsis-time-prediction", "version": "2.0.0"}
